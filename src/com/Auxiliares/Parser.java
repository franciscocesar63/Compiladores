...
public class Parser {
    ...
    private static String posterior = "";
    ...
    private boolean UE() throws IOException, LexerException {
        ...
        } else if (match(Token.ID) || match(Token.LITERALNUMERICO)) { //como far�amos para ele aceitar literal num�rico?

            resultado = true;

        } else if (match(Token.OP, Token.AD) || match(Token.OP, Token.SUB) || match(Token.LOG, Token.NOT) || match(Token.OP, Token.INC)) {
    ...
    private boolean match(int tipoToken) throws IOException, LexerException {
        ...
        if (token.getTipo() == tipoToken) {
            ...
            if (tipoToken == Token.LITERALNUMERICO) {
                ...
                if (anterior.equalsIgnoreCase("+") || anterior.equalsIgnoreCase("++") || anterior.equalsIgnoreCase("-")
                        || anterior.equalsIgnoreCase("--") && !anterior.equalsIgnoreCase(" ")) {
                    mensagemErro = "Token não esperado: " + token.getValor();
                    System.err.println(mensagemErro);

                } else {
                    posterior = token.getValor().toString();
            ...
            switch (tipoToken) {
                ...
                case Token.OP: {
                    ...
                    switch (valorToken) {
                        ...
                        case Token.AD:
                            if (posterior.contains("1") || posterior.contains("2") || posterior.contains("3") || posterior.contains("4") || posterior.contains("5")
                                    || posterior.contains("6") || posterior.contains("7")
                                    || posterior.contains("8") || posterior.contains("9") || posterior.contains("0") && !posterior.isEmpty()) {
